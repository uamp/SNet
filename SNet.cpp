		...
		if(radio.receiveDone()){
			if(radio.DATALEN==(sizeof(payload_t)+sizeof(RF24NetworkHeader))){
				...
				for (byte i = 0; i < radio.DATALEN; i++){
					...
					if(i<sizeof(RF24NetworkHeader)){
						memcpy (&header+i,&temp_byte,1);    //header
					}else{
						memcpy (&payload+i-sizeof(RF24NetworkHeader),&temp_byte,1);  //main data load
					}
				}
...
bool SNet::write(RF24NetworkHeader header, payload_t payload_to_send){
	#ifdef USE_NRF24
		bool ok = network.write(header,&payload_to_send,sizeof(payload_to_send));
	#endif
	#ifdef USE_RFM69
		uint8_t buffer[RF69_MAX_DATA_LEN];
		header.id=_msg_count++;  //dealt with automatically for NRF24
		memcpy(&buffer,&header,sizeof(RF24NetworkHeader));
		memcpy(&buffer+sizeof(RF24NetworkHeader),&payload_to_send,sizeof(payload_t));
		bool ok = radio.sendWithRetry(header.to_node, buffer, sizeof(RF24NetworkHeader)+sizeof(payload_t));
	#endif
	return ok;
}
